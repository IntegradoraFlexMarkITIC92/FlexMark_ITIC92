...
if (!isset($_SESSION["logged_adm"])){
   ...
   if(!is_null($_REQUEST['nuevaDepto']) && $_REQUEST['nuevaDepto']=="ADD"){      
      //Llamo la funcion de actualizar            
      $nombre=$_REQUEST['nombre'];
	  #$status=$_REQUEST['status'];
	  #$idDepto=$_REQUEST['idDepto'];
           
      addConfDepartamento($nombre,$status);

    }

    if(!is_null($_REQUEST['STATUS']) && $_REQUEST['STATUS']!="" && !is_null($_REQUEST['IDE']) && $_REQUEST['IDE']!=""){      
      cambiarStatusDepartamento($_REQUEST['IDE'],$_REQUEST['STATUS']);
    }

    if(!is_null($_REQUEST['updateDE']) && $_REQUEST['updateDE']=="updateDE"){      
      //Llamo la funcion de actualizar  
	  $idDep=$_REQUEST['updateIdDep'];
      $nombre=$_REQUEST['nombre'];
      #$status=$_REQUEST['status'];     
      
      updateDepartamento($idDep,$nombre,$status);
...
<html lang="en">
  ...
  <h1>Administracion Departamentos.</h1>    
  <br>
    ...
    if(!is_null($_REQUEST['ADD']) && $_REQUEST['ADD']=="true"){
      // Aqui va la funcion..
        displayNewDepartamento();
    }else if (!is_null($_REQUEST['ID'])) {
        displayUpdateDepartamento($_REQUEST['ID']);
    }
